...
- _2025-12-20T00:15:00Z Update_: Added a reusable k6 scenario (`loadtest/k6/command-pipeline.js`) plus Docker Compose + Kubernetes manifests (`platform/kubernetes/loadtest`) so we can exercise the event-driven gateway at 20k ops/sec. Updated `docs/TESTING_ENVIRONMENT.md` with the new workflow and ensured Locust shares the same routes/env wiring.
