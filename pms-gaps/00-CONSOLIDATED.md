...
| [WS-04](WORKSTREAMS.md#ws-04) Reversals + bulk | 6/20 — PMS-02-01, 02-14, 01-20, 01-21, 01-22, 02-05 | Undo check-in, undo check-out, reinstate a cancellation. Each takes a mandatory `reason_code` resolved against `reason_codes` — a table that had existed with no rows, no route and no reader. Reinstatement re-acquires the availability hold *before* changing status, so it fails closed rather than overbooking. Mass cancel, mass check-in and mass update then ride one `BatchCommand<T>` envelope — one transaction per target, every requested item accounted for in `command_batch_items`, and each mass command is its single command applied N times so the two cannot drift. WS-15's group bulk actions reuse it. |
...
| [WS-04](WORKSTREAMS.md#ws-04) | Lifecycle reversals and bulk operations | 6/20 | 1 | **Reversals done; batch envelope written once and in use.** `buildBatchCommandSchema` + `runBatchCommand` carry mass cancel, mass check-in and mass update, and WS-15's group bulk actions reuse them. The remaining P0 is room move for an in-house guest. |
