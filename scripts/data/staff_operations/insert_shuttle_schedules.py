...
def insert_shuttle_schedules(conn, schedules_per_property: int = 2):
    ...
    for property_rec in data_store["properties"]:
        ...
        for index in range(schedules_per_property):
            ...
            default_vehicle = random.choice(property_vehicles)["vehicle_id"] if property_vehicles else None
            alternate_vehicle_ids = None
            if property_vehicles:
                alternates = [v["vehicle_id"] for v in property_vehicles if v["vehicle_id"] != default_vehicle]
                alternate_vehicle_ids = alternates[:2] if alternates else None
            ...
            cur.execute(
                ...
                INSERT INTO shuttle_schedules (
                    ...
                    schedule_name, schedule_code, description,
                    route_type, route_name, is_roundtrip, is_loop,
                    route_stops, total_stops,
                    departure_location, departure_address, departure_coordinates,
                    destination_location, destination_address, destination_coordinates,
                    estimated_duration_minutes, estimated_distance_km, buffer_time_minutes,
                    schedule_type, departure_times, interval_minutes, first_departure_time, last_departure_time,
                    operates_monday, operates_tuesday, operates_wednesday, operates_thursday,
                    operates_friday, operates_saturday, operates_sunday,
                    operates_holidays, operates_on_closure_days, special_operating_dates, excluded_dates,
                    seasonal, season_start_date, season_end_date, season_name,
                    default_vehicle_id, alternate_vehicle_ids, vehicle_type_required, min_vehicle_capacity,
                    max_passengers_per_trip, wheelchair_spots_available, standee_capacity, reservation_required,
                    walk_on_allowed, advance_booking_hours,
                    service_type, price_per_person, child_price, roundtrip_price, currency_code,
                    guest_only, public_access, minimum_age, requires_room_key,
                    schedule_status, active_from_date, active_to_date,
                    real_time_tracking_enabled, tracking_available_to_guests, tracking_url_template,
                    automated_notifications, sms_reminders, email_confirmations, notification_minutes_before,
                    boarding_location, boarding_instructions, check_in_required, check_in_minutes_before,
                    average_occupancy_percent, total_trips_ytd, total_passengers_ytd, no_show_rate_percent,
                    on_time_performance_percent, average_delay_minutes, guest_satisfaction_rating,
                    displayed_on_website, displayed_on_app, available_for_booking, booking_url, external_booking_system,
                    wifi_available, air_conditioning, restroom_available, refreshments_provided, luggage_storage,
                ...
                (
                    ...
                    schedule_name,
                    schedule_code,
                    f"{config['name']} serving {property_name}",
                    config["route_type"],
                    route_name,
                    True,
                    False,
                    ...
                    len(route_stops),
                    departure_location,
                    f"{property_name} shuttle departure",
                    json.dumps(
                        {
                            "lat": float(fake.latitude()),
                            "lng": float(fake.longitude()),
                    ...
                    last_departure_time,
                    operates_matrix["operates_monday"],
                    operates_matrix["operates_tuesday"],
                    operates_matrix["operates_wednesday"],
                    operates_matrix["operates_thursday"],
                    operates_matrix["operates_friday"],
                    operates_matrix["operates_saturday"],
                    operates_matrix["operates_sunday"],
                    operates_holidays,
