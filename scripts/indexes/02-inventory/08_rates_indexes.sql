...
CREATE INDEX IF NOT EXISTS idx_rates_property_code ON rates(tenant_id, property_id, rate_code) WHERE deleted_at IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_rates_date_range ON rates(tenant_id, valid_from, valid_until) WHERE deleted_at IS NULL AND status = 'ACTIVE';

-- Composite index for rate search (most common query)
CREATE INDEX IF NOT EXISTS idx_rates_property_type_dates ON rates(tenant_id, property_id, room_type_id, valid_from, valid_until, status)
...
-- Booking window
CREATE INDEX IF NOT EXISTS idx_rates_advance_booking ON rates(tenant_id, advance_booking_days_min, advance_booking_days_max) WHERE deleted_at IS NULL;

-- Stay restrictions
CREATE INDEX IF NOT EXISTS idx_rates_length_of_stay ON rates(tenant_id, min_length_of_stay, max_length_of_stay) WHERE deleted_at IS NULL;
