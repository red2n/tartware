...
BEGIN
    ...
    WHERE constraint_type = 'FOREIGN KEY'
        AND table_name IN ('room_types', 'rooms', 'rates', 'room_availability')
        AND tc.table_schema IN ('public', 'availability');
    ...
    IF v_restrict_count = v_total_fks AND v_cascade_count = v_total_fks THEN
        RAISE NOTICE '✓✓✓ ROOM & INVENTORY MANAGEMENT CONSTRAINT VERIFICATION PASSED ✓✓✓';
    ELSE
        RAISE WARNING '⚠⚠⚠ ROOM & INVENTORY MANAGEMENT CONSTRAINT POLICY VIOLATIONS FOUND ⚠⚠⚠';
    END IF;
END $$;

\echo ''
\echo '=============================================='
\echo 'Room & Inventory Management constraint verification complete!'
\echo '=============================================='
