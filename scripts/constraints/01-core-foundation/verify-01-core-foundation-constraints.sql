...
BEGIN
    ...
    WHERE constraint_type = 'FOREIGN KEY'
        AND table_name IN ('tenants', 'users', 'user_tenant_associations', 'properties', 'guests')
        AND tc.table_schema = 'public';
    ...
    IF v_restrict_count = v_total_fks AND v_cascade_count = v_total_fks THEN
        RAISE NOTICE '✓✓✓ CORE FOUNDATION CONSTRAINT VERIFICATION PASSED ✓✓✓';
    ELSE
        RAISE WARNING '⚠⚠⚠ CORE FOUNDATION CONSTRAINT POLICY VIOLATIONS FOUND ⚠⚠⚠';
    END IF;
END $$;

\echo ''
\echo '=============================================='
\echo 'Core Foundation constraint verification complete!'
\echo '=============================================='
