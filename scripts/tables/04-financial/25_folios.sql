...
CREATE TABLE folios (
    ...
    -- Check Constraints
    -- Folio accounting identity: charges minus payments minus credits sums to the running balance.
    -- This supports signed balances where overpayments reflect as negative values (USALI 12th Ed §7.3).
    -- The credit_balance column is retired in favor of signed 'balance'.
...
COMMENT ON COLUMN folios.balance IS 'Current running balance (signed). Negative values indicate guest credit (overpayment).';
