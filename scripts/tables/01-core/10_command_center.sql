...
WITH seed_commands(command_name, description, default_target_service, required_modules) AS (
    VALUES
        ...
        ('billing.cashier.open', 'Open a new cashier/till session', 'cashier-service', ARRAY['finance-automation']),
        ('billing.cashier.close', 'Close and reconcile a cashier session', 'cashier-service', ARRAY['finance-automation']),
        ...
        ('billing.cashier.handover', 'Close outgoing cashier session and open incoming session atomically', 'cashier-service', ARRAY['finance-automation']),
