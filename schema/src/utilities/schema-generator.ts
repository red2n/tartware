...
function mapPostgresToZod(dataType: string, udtName: string, isNullable: boolean): string {
  let zodType: string;
...
async function generateSchema(tableName: string, category: string): Promise<void> {
  ...
  for (const col of columns) {
    ...
    const zodType = mapPostgresToZod(data_type, udt_name, is_nullable === 'YES');
...
async function main() {
  ...
  if (!CATEGORY_TABLES[category]) {
    ...
    console.error(`Available categories: ${Object.keys(CATEGORY_TABLES).join(', ')}`);
    process.exit(1);
  }
