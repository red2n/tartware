...
import { uuid, isoDateString } from "../../shared/base-schemas.js";

/**
 * Guest booking history for personalization
 */
export const GuestBookingHistorySchema = z.object({
  totalBookings: z.number().int().nonnegative().describe("Total number of past bookings"),
  previousRoomTypes: z.array(z.string()).describe("Room types previously booked"),
  previousProperties: z.array(z.string()).describe("Properties previously stayed at"),
  averageRate: z.number().nonnegative().describe("Average nightly rate paid"),
  preferredAmenities: z.array(z.string()).describe("Frequently requested amenities"),
});
...
export const RecommendationGuestPreferencesSchema = z.object({
  preferredRoomTypes: z.array(z.string()).optional().describe("Preferred room types"),
  floorPreference: z.enum(["high", "low", "any"]).optional().describe("Preferred floor"),
  viewPreferences: z.array(z.string()).optional().describe("Preferred view types"),
...
export type RecommendationGuestPreferences = z.infer<typeof RecommendationGuestPreferencesSchema>;
...
export const RoomRecommendationQuerySchema = z.object({
  ...
  adults: z.number().int().min(1).max(10).default(1).describe("Number of adults"),
  children: z.number().int().min(0).max(10).default(0).describe("Number of children"),
  limit: z.number().int().min(1).max(50).optional().describe("Maximum results to return"),
});

export type RoomRecommendationQuery = z.infer<typeof RoomRecommendationQuerySchema>;
...
export const RoomRecommendationSchema = z.object({
  ...
  totalPrice: z.number().nonnegative().optional().describe("Total price for the stay"),
  ...
  maxOccupancy: z.number().int().positive().optional().describe("Maximum occupancy"),
  bedType: z.string().optional().describe("Bed type"),
  squareFootage: z.number().positive().optional().describe("Square footage"),
  isUpgrade: z.boolean().optional().describe("Whether this is an upgrade opportunity"),
  upgradeDiscount: z.number().min(0).max(100).optional().describe("Upgrade discount percentage"),
  relevanceScore: z.number().min(0).max(1).describe("Relevance score (0-1, higher is better)"),
  source: z.enum(["available_rooms", "similar_rooms", "upgrade_opportunity"]).describe("Candidate source"),
});
...
export const RoomRecommendationResponseSchema = z.object({
  ...
  recommendations: z.array(RoomRecommendationSchema).describe("Recommended rooms"),
  totalCandidates: z.number().int().nonnegative().describe("Total candidates considered"),
  executionTimeMs: z.number().nonnegative().describe("Pipeline execution time in ms"),
});

export type RoomRecommendationResponse = z.infer<typeof RoomRecommendationResponseSchema>;

/**
 * Pipeline metrics for observability
 */
export const RecommendationPipelineMetricsSchema = z.object({
  totalDurationMs: z.number().nonnegative().describe("Total execution time in ms"),
  stageDurations: z.record(z.string(), z.number()).describe("Time spent in each stage"),
  candidateCounts: z.object({
    sourced: z.number().int().nonnegative().describe("Candidates from sources"),
    afterHydration: z.number().int().nonnegative().describe("Candidates after hydration"),
    afterFiltering: z.number().int().nonnegative().describe("Candidates after filtering"),
    selected: z.number().int().nonnegative().describe("Final selected candidates"),
  }),
...
export type RecommendationPipelineMetrics = z.infer<typeof RecommendationPipelineMetricsSchema>;
