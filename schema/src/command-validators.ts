...
const commandPayloadValidators = new Map<string, CommandPayloadValidator>([
	...
	["billing.ar.post", (payload) => BillingArPostCommandSchema.parse(payload)],
	[
		"billing.ar.apply_payment",
		(payload) => BillingArApplyPaymentCommandSchema.parse(payload),
	],
	["billing.ar.age", (payload) => BillingArAgeCommandSchema.parse(payload)],
	[
	...
	[
		...
		(payload) => BillingTaxConfigCreateCommandSchema.parse(payload),
	],
	[
		"billing.tax_config.update",
		(payload) => BillingTaxConfigUpdateCommandSchema.parse(payload),
	],
	[
		"billing.tax_config.delete",
		(payload) => BillingTaxConfigDeleteCommandSchema.parse(payload),
	],
	[
		"billing.express_checkout",
		(payload) => BillingExpressCheckoutCommandSchema.parse(payload),
	],
	[
		"billing.cashier.handover",
		(payload) => BillingCashierHandoverCommandSchema.parse(payload),
	],
	[
		"billing.folio.create",
		(payload) => BillingFolioCreateCommandSchema.parse(payload),
	],
	[
		"billing.invoice.void",
		(payload) => BillingInvoiceVoidCommandSchema.parse(payload),
	],
	[
		"billing.credit_note.create",
		(payload) => BillingCreditNoteCreateCommandSchema.parse(payload),
	],
	[
		"billing.invoice.reopen",
		(payload) => BillingInvoiceReopenCommandSchema.parse(payload),
	],
	[
		"billing.folio.reopen",
		(payload) => BillingFolioReopenCommandSchema.parse(payload),
	],
	[
		"billing.folio.merge",
		(payload) => BillingFolioMergeCommandSchema.parse(payload),
	],
	[
		"billing.chargeback.update_status",
		(payload) => BillingChargebackUpdateStatusCommandSchema.parse(payload),
	],
	[
		"billing.no_show.charge",
		(payload) => BillingNoShowChargeCommandSchema.parse(payload),
	],
	[
		"billing.late_checkout.charge",
		(payload) => BillingLateCheckoutChargeCommandSchema.parse(payload),
	],
	[
		"billing.tax_exemption.apply",
		(payload) => BillingTaxExemptionApplyCommandSchema.parse(payload),
	],
	[
		"billing.cancellation.penalty",
		(payload) => BillingCancellationPenaltyCommandSchema.parse(payload),
	],
	[
		"billing.comp.post",
	...
	["group.create", (payload) => GroupCreateCommandSchema.parse(payload)],
	["group.add_rooms", (payload) => GroupAddRoomsCommandSchema.parse(payload)],
	[
