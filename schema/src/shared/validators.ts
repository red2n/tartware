 ...
 */
export const validateDateRange = <T extends { start_date: Date; end_date: Date }>(
  data: T,
 ...
 */
export const validateMinimumStay = (checkIn: Date, checkOut: Date, minNights: number): boolean => {
 ...
 */
export const validateAdvanceBooking = (checkIn: Date, maxDaysInAdvance: number): boolean => {
  const now = new Date();
  const daysInAdvance = (checkIn.getTime() - now.getTime()) / (1000 * 60 * 60 * 24);
 ...
 */
export const validateRateAmount = (amount: number, maxAmount: number = 100000): boolean => {
 ...
 */
export const validateCancellationDeadline = (deadline: Date, checkIn: Date): boolean => {
 ...
 */
export const zodDateRangeRefinement = (message: string = 'End date must be after start date') => ({
  validator: (data: { start_date: Date; end_date: Date }) => data.end_date > data.start_date,
  message,
});
