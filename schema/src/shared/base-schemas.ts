...
export const auditTimestamps = {
  ...
  updated_at: z.date().optional().describe('Timestamp when record was last updated'),
 ...
 * URL/URI pattern
 */
export const url = z.string().url({ message: 'Invalid URL format' }).describe('Valid URL');
...
export const currencyCode = z
  ...
  .length(3)
  .toUpperCase()
  .regex(/^[A-Z]{3}$/, { message: 'Currency code must be 3 uppercase letters (ISO 4217)' })
...
export const countryCode = z
  ...
  .length(2)
  .toUpperCase()
  .regex(/^[A-Z]{2}$/, { message: 'Country code must be 2 uppercase letters (ISO 3166-1)' })
...
export const languageCode = z
  ...
  .length(2)
  .toLowerCase()
  .regex(/^[a-z]{2}$/, { message: 'Language code must be 2 lowercase letters (ISO 639-1)' })
...
export const confirmationNumber = z
  ...
  .max(20)
  .toUpperCase()
  .regex(/^[A-Z0-9-]+$/, { message: 'Confirmation number must be alphanumeric' })
