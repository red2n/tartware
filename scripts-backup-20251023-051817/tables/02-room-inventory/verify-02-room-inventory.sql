...
BEGIN
    ...
    -- Check room_availability in availability schema
    IF EXISTS (
        SELECT 1 FROM information_schema.tables
        ...
        v_found_count := v_found_count + 1;
        RAISE NOTICE '  ✓ room_availability exists (in availability schema)';
    ELSE
        v_missing_tables := array_append(v_missing_tables, 'availability.room_availability');
        RAISE WARNING '  ✗ room_availability is MISSING';
    END IF;
...
\echo ''
\echo '=============================================='
\echo 'Room & Inventory Management verification complete!'
\echo '=============================================='
