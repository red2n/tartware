...
process.env.SERVICE_NAME =
  process.env.SERVICE_NAME ?? "@tartware/availability-guard-service";
...
const kafkaFailoverEnabled = toBoolean(
...
const kafka = {
  ...
  reservationEventsTopic:
    process.env.RESERVATION_EVENTS_TOPIC ?? "reservations.events",
  inventoryEventsTopic:
    process.env.INVENTORY_EVENTS_TOPIC ?? "inventory.events.shadow",
  inventoryDlqTopic:
    process.env.INVENTORY_EVENTS_DLQ_TOPIC ?? "inventory.events.dlq",
  commandTopic: process.env.COMMAND_CENTER_TOPIC ?? "commands.primary",
  consumerGroupId:
    process.env.AVAILABILITY_GUARD_CONSUMER_GROUP ??
...
const consumerEmailChannel = {
  enabled: toBoolean(
    process.env.AVAILABILITY_GUARD_NOTIFICATION_EMAIL_ENABLED,
    true,
  ),
  webhookUrl:
    process.env.AVAILABILITY_GUARD_NOTIFICATION_EMAIL_WEBHOOK_URL ?? "",
  apiKey: process.env.AVAILABILITY_GUARD_NOTIFICATION_EMAIL_API_KEY,
  timeoutMs: toNumber(
    process.env.AVAILABILITY_GUARD_NOTIFICATION_EMAIL_TIMEOUT_MS,
    7000,
  ),
};
...
const consumerSmsChannel = {
  enabled: toBoolean(
    process.env.AVAILABILITY_GUARD_NOTIFICATION_SMS_ENABLED,
    false,
  ),
  ...
  timeoutMs: toNumber(
    process.env.AVAILABILITY_GUARD_NOTIFICATION_SMS_TIMEOUT_MS,
    5000,
  ),
};
...
const consumerSlackChannel = {
  ...
  webhookUrl:
    process.env.AVAILABILITY_GUARD_NOTIFICATION_SLACK_WEBHOOK_URL ?? "",
  apiKey: process.env.AVAILABILITY_GUARD_NOTIFICATION_SLACK_API_KEY,
  timeoutMs: toNumber(
    process.env.AVAILABILITY_GUARD_NOTIFICATION_SLACK_TIMEOUT_MS,
    5000,
  ),
};
...
const manualReleaseConfig = {
  ...
  notifications: {
    enabled: toBoolean(
      process.env.AVAILABILITY_GUARD_NOTIFICATIONS_ENABLED,
      true,
    ),
    topic:
      process.env.AVAILABILITY_GUARD_NOTIFICATION_TOPIC ??
      "availability-guard.notifications",
    recipients: parseList(
      process.env.AVAILABILITY_GUARD_NOTIFICATION_RECIPIENTS,
    ),
    consumer: {
      enabled: toBoolean(
        process.env.AVAILABILITY_GUARD_NOTIFICATION_CONSUMER_ENABLED,
        true,
      ),
      groupId:
...
const guard = {
  ...
  defaultTtlSeconds: toNumber(
    process.env.LOCK_DEFAULT_TTL_SECONDS,
    2 * 60 * 60,
  ),
