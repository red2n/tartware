...
const primeRegistryWithRetry = async (): Promise<void> => {
  ...
  for (;;) {
    try {
      await refreshRegistry();
      if (attempt > 0) {
        registryLogger.info({ attempt }, "command registry refreshed after retries");
      }
      return;
    } catch (error) {
      attempt += 1;
      if (maxRetries >= 0 && attempt > maxRetries) {
        registryLogger.error({ attempt, maxRetries }, "exhausted command registry startup retries");
        throw error;
      }
      const delayMs = commandRegistryConfig.startupRetryDelayMs * Math.max(1, attempt);
      registryLogger.warn(
        {
          attempt,
          delayMs,
