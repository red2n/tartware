...
export const gatewayConfig = {
  ...
  rateLimit: {
    enabled: !parseBooleanEnv(env.API_GATEWAY_DISABLE_RATE_LIMIT, false),
    /** Global default — applies to most read endpoints. Increased for E2E tests. */
    max: Number(env.API_GATEWAY_RATE_MAX ?? 1000),
    timeWindow: env.API_GATEWAY_RATE_WINDOW ?? "1 minute",
    /** Tighter limit for command (write) endpoints. Increased for E2E tests. */
    commandMax: Number(env.API_GATEWAY_RATE_COMMAND_MAX ?? 500),
