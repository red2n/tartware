...
process.env.SERVICE_NAME = process.env.SERVICE_NAME ?? "@tartware/finance-admin-service";
...
const kafka = resolveKafkaConfig({
  clientId: process.env.KAFKA_CLIENT_ID ?? "tartware-finance-admin-service",
...
const commandCenter = {
  ...
  consumerGroupId:
    process.env.COMMAND_CENTER_CONSUMER_GROUP ?? "finance-admin-command-center-consumer",
  targetServiceId: process.env.COMMAND_CENTER_TARGET_SERVICE_ID ?? "finance-admin-service",
...
export const config = {
  ...
  port: parseNumberEnv(process.env.PORT, 3090),
  log: {
