  ...
  app.addHook(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        // Get tenantId from x-tenant-id header (preferred) or JWT claim
        const tenantIdHeader = request.headers["x-tenant-id"];
        const tenantId = (Array.isArray(tenantIdHeader) ? tenantIdHeader[0] : tenantIdHeader) || decoded.tenantId;
