...
export const registerRegistryRoutes = (app: FastifyInstance): void => {
  ...
  app.put<{ Body: HeartbeatBody }>("/v1/registry/heartbeat", async (request, reply) => {
    ...
    if (!instance) {
      // Registry was restarted — service is still alive, just not in the in-memory store.
      // Return 200 so the service doesn't error; it will re-register on its next startup.
      return reply.send({ name, port, status: "unknown" });
    }
