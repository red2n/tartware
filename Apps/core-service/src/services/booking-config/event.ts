...
const mapEventBookingDetailRow = (row: EventBookingDetailRow): EventBookingDetail => {
  return EventBookingDetailSchema.parse({
    ...
    // Billing basis — the columns deriveEventChargeQuote prices from, so the
    // screen's preview is computed from the same numbers billing-service will.
    setup_fee: toNumber(row.setup_fee),
    equipment_rental_fee: toNumber(row.equipment_rental_fee),
    av_equipment_fee: toNumber(row.av_equipment_fee),
    labor_charges: toNumber(row.labor_charges),
    estimated_food_beverage: toNumber(row.estimated_food_beverage),
    service_charge_percent: toNumber(row.service_charge_percent),
    tax_rate: toNumber(row.tax_rate),
    discount_amount: toNumber(row.discount_amount),
    tax_exempt: Boolean(row.tax_exempt),
    charges_posted_at: toIsoString(row.charges_posted_at) ?? null,
    folio_number: row.folio_number,
    folio_status: row.folio_status,
    folio_balance: toNumber(row.folio_balance),
  });
};
  ...
  const { rows } = await query<{ event_id: string }>(
    `
      INSERT INTO public.event_bookings (
        ...
        rental_rate, setup_fee, equipment_rental_fee, av_equipment_fee,
        labor_charges, estimated_food_beverage, service_charge_percent,
        tax_rate, discount_amount, tax_exempt,
        estimated_total, deposit_required, currency_code,
        ...
        $34, $35, $36, $37,
        $38, $39, $40,
        $41, $42, COALESCE($43, false),
        $44, $45, COALESCE($46, 'USD'),
        $47::uuid, $48, $49, $50,
        $51, $51
      )
      RETURNING event_id
    ...
    [
      ...
      input.setupFee ?? null,
      input.equipmentRentalFee ?? null,
      input.avEquipmentFee ?? null,
      input.laborCharges ?? null,
      input.estimatedFoodBeverage ?? null,
      input.serviceChargePercent ?? null,
      input.taxRate ?? null,
  ...
  const { rowCount } = await query(
    `
      ...
      SET
        ...
        setup_fee = COALESCE($34, setup_fee),
        equipment_rental_fee = COALESCE($35, equipment_rental_fee),
        av_equipment_fee = COALESCE($36, av_equipment_fee),
        labor_charges = COALESCE($37, labor_charges),
        estimated_food_beverage = COALESCE($38, estimated_food_beverage),
        service_charge_percent = COALESCE($39, service_charge_percent),
        tax_rate = COALESCE($40, tax_rate),
        discount_amount = COALESCE($41, discount_amount),
        tax_exempt = COALESCE($42, tax_exempt),
        estimated_total = COALESCE($43, estimated_total),
        deposit_required = COALESCE($44, deposit_required),
        currency_code = COALESCE($45, currency_code),
        folio_id = COALESCE($46::uuid, folio_id),
        billing_instructions = COALESCE($47, billing_instructions),
        billing_contact_name = COALESCE($48, billing_contact_name),
        billing_contact_email = COALESCE($49, billing_contact_email),
        updated_by = $50,
        updated_at = CURRENT_TIMESTAMP
      WHERE event_id = $1::uuid AND tenant_id = $2::uuid
        AND COALESCE(is_deleted, false) = false
