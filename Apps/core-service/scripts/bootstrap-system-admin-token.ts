...
const log = (message: string) => process.stdout.write(`[bootstrap-admin] ${message}\n`);
const warn = (message: string) => process.stderr.write(`[bootstrap-admin][warn] ${message}\n`);
  ...
  if (rows.length === 0) {
    ...
    await client.query(
      ...
      [
        ...
        mfaSecret,
        ips,
        JSON.stringify({ trusted_devices: ["bootstrap"] }),
        now,
      ],
    );
...
const main = async () => {
  log("Connecting to configured target database.");
