...
process.env.SERVICE_NAME = process.env.SERVICE_NAME ?? "@tartware/guest-experience-service";
...
const kafka = resolveKafkaConfig({
  clientId: process.env.KAFKA_CLIENT_ID ?? "tartware-guest-experience-service",
...
const commandCenter = {
  ...
  consumerGroupId:
    process.env.COMMAND_CENTER_CONSUMER_GROUP ?? "guest-experience-command-center-consumer",
  targetServiceId: process.env.COMMAND_CENTER_TARGET_SERVICE_ID ?? "guest-experience-service",
