...
import {
  countGuestRedemptions,
  countRedemptionsForReward,
  findLoyaltyBalance,
  findReward,
  selectGuestRedemptions,
...
export async function redeemReward(params: {
  ...
  // Load reward
  const rewardResult = await findReward(params.rewardId, params.tenantId);
  ...
  const programResult = await findLoyaltyBalance(params.guestId, params.tenantId);
  ...
  if (reward.max_redemptions_per_guest) {
    const guestCountResult = await countRedemptionsForReward(
      params.rewardId,
      params.guestId,
      params.tenantId,
    );
...
export async function listGuestRedemptions(params: {
  ...
  const countResult = await countGuestRedemptions(params.tenantId, params.guestId);

  const result = await selectGuestRedemptions(params.tenantId, params.guestId, limit, offset);
