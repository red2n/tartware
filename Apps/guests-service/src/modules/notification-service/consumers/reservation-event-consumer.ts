...
const RESERVATION_PROPERTY_LOOKUP_SQL = `
  SELECT property_id
...
const resolveEventPropertyId = async (event: ReservationEvent): Promise<string | null> => {
  const fromEvent = toUuidOrNull(event.propertyId);
  if (fromEvent) {
    return fromEvent;
  }

  const reservationId = toUuidOrNull(event.reservationId);
  const tenantId = toUuidOrNull(event.tenantId);
  if (!reservationId || !tenantId) {
    return null;
  }

  const { rows } = await query<{ property_id: string }>(RESERVATION_PROPERTY_LOOKUP_SQL, [
    tenantId,
    reservationId,
  ]);
  return toUuidOrNull(rows[0]?.property_id);
};
...
const toReservationEvent = (envelope: ReservationEventEnvelope): ReservationEvent => {
  ...
  return {
    ...
    propertyId:
      readNonEmptyString(envelope.payload.property_id) ??
      readFromPayloadOrMetadata(envelope.payload, "property_id"),
...
const processReservationEvent = async (event: ReservationEvent): Promise<void> => {
  const propertyId = await resolveEventPropertyId(event);
  const guestId = toUuidOrNull(event.guestId);

  if (!propertyId) {
    logger.warn(
      { eventType: event.eventType, reservationId: event.reservationId },
      "Skipping reservation notification dispatch due to missing property_id",
    );
  }
  ...
  if (!guestId) {
    logger.warn(
      { eventType: event.eventType, reservationId: event.reservationId },
      "Skipping reservation notification dispatch due to missing guest_id",
    );
  }
