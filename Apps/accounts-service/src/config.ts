...
process.env.SERVICE_NAME = process.env.SERVICE_NAME ?? "@tartware/accounts-service";
...
const kafka = resolveKafkaConfig({
  clientId: process.env.KAFKA_CLIENT_ID ?? "tartware-accounts-service",
  defaultPrimaryBroker: "localhost:29092",
});

const commandCenter = {
  topic: process.env.COMMAND_CENTER_TOPIC ?? "commands.primary",
  consumerGroupId: process.env.COMMAND_CENTER_CONSUMER_GROUP ?? "accounts-command-center-consumer",
  targetServiceId: process.env.COMMAND_CENTER_TARGET_SERVICE_ID ?? "accounts-service",
