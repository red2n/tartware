...
const EMAIL_REGEX = /[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}(?:\.[A-Z]{2,})?/i;
...
const buildValueTesterList = (additional?: SensitiveValueTester[]): SensitiveValueTester[] => {
...
const shouldRedactString = (value: string, testers: SensitiveValueTester[]): boolean =>
  testers.some((tester) => tester(value));
...
export const createLogSanitizer = (options?: LogSanitizerOptions) => {
  ...
  const sanitizeInternal = (value: unknown, depth: number, visited: WeakSet<object>): unknown => {
    ...
    try {
      if (Array.isArray(value)) {
        return value.map((entry) => sanitizeInternal(entry, depth + 1, visited));
      }
...
export const sanitizeLogValue = (value: unknown, options?: LogSanitizerOptions): unknown =>
  createLogSanitizer(options)(value);
...
export interface RequestLoggingOptions extends LogSanitizerOptions {
  ...
  buildRequestContext?: (request: FastifyRequest) => Record<string, unknown> | undefined;
...
const STANDARD_REQUEST_LOG_SENSITIVE_KEYS = Array.from(
  new Set([...DEFAULT_SENSITIVE_LOG_KEYS, ...ADDITIONAL_REQUEST_LOG_SENSITIVE_KEYS]),
);
...
export const buildSecureRequestLoggingOptions = (
  ...
  if (overrides?.sensitiveKeys?.length) {
    merged.sensitiveKeys = Array.from(
      new Set([...(base.sensitiveKeys ?? []), ...overrides.sensitiveKeys]),
    );
  } else {
  ...
  if (overrides?.valueTesters?.length) {
    merged.valueTesters = [...(base.valueTesters ?? []), ...overrides.valueTesters];
  } else {
...
export const withRequestLogging = (app: FastifyAppLike, options?: RequestLoggingOptions): void => {
  ...
  const shouldSkip = (request: FastifyRequest) => Boolean(options?.skip?.(request));
  ...
  if (logOnResponse) {
    app.addHook("onResponse", async (request, reply) => {
      ...
      request.log.info(payload, options?.responseMessage ?? "request completed");
    });
  }
};
...
export const initTelemetry = async (options: TelemetryOptions): Promise<NodeSDK | undefined> => {
  ...
  diag.setLogger(new DiagConsoleLogger(), resolveDiagLevel(process.env.OTEL_LOG_LEVEL));
  ...
  if (!traceEndpoint && !logEndpoint) {
    console.info("[telemetry] No OTLP endpoints configured - telemetry disabled.");
    return undefined;
  }
...
const shouldPrettyPrint = (
  ...
  const resolvedEnvironment = (environment ?? process.env.NODE_ENV ?? "development").toLowerCase();
...
const mapSeverity = (level: unknown): { severityText: string; severityNumber: SeverityNumber } => {
