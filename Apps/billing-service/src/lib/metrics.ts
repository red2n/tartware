...
const commandOutcomeCounter = new Counter({
  name: "billing_command_outcomes_total",
  help: "Count of billing commands processed by outcome",
  labelNames: ["command", "status"] as const,
  registers: [metricsRegistry],
});

const commandDurationHistogram = new Histogram({
  name: "billing_command_processing_duration_seconds",
  help: "Duration of billing command processing",
...
const commandConsumerLagGauge = new Gauge({
  name: "billing_command_consumer_lag",
