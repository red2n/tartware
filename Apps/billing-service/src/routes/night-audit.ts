    ...
    async (request) => {
      ...
      const result = await query<{
        ...
        `SELECT
           ...
           COUNT(nal.audit_log_id)::int AS total_steps,
           COUNT(nal.audit_log_id) FILTER (WHERE nal.step_status = 'COMPLETED')::int AS steps_completed,
           COUNT(nal.audit_log_id) FILTER (WHERE nal.step_status = 'FAILED')::int AS steps_failed,
