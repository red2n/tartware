...
const buildKafkaClient = (logger: KafkaLogger) =>
  new Kafka({
    clientId: config.roll.kafka.clientId,
    brokers: config.roll.kafka.brokers,
    logCreator: createPinoLogCreator(logger),
  });
...
class RollLifecycleConsumer {
  ...
  constructor(logger: FastifyBaseLogger) {
    ...
    this.kafka = buildKafkaClient(this.logger);
  }
  ...
  async start(): Promise<void> {
    ...
    this.consumer = this.kafka.consumer({
      groupId: config.roll.kafka.consumerGroupId,
      allowAutoTopicCreation: false,
      maxBytesPerPartition: config.roll.kafka.maxBatchBytes,
    });

    await this.consumer.connect();
    await this.consumer.subscribe({
      topic: config.roll.kafka.topic,
      fromBeginning: false,
    });
    ...
    this.running = true;
    this.logger.info(
      { topic: config.roll.kafka.topic, groupId: config.roll.kafka.consumerGroupId },
  ...
  private async persistConsumerOffset({
    ...
    await upsertConsumerOffset({
      consumerGroup: config.roll.kafka.consumerGroupId,
      topic: batch.topic,
