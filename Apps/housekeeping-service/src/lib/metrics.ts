...
const commandOutcomeCounter = new Counter({
  name: "housekeeping_command_outcomes_total",
  help: "Count of housekeeping commands processed by outcome",
  labelNames: ["command", "status"] as const,
  registers: [metricsRegistry],
});

const commandDurationHistogram = new Histogram({
  name: "housekeeping_command_processing_duration_seconds",
  help: "Duration of housekeeping command processing",
...
const commandConsumerLagGauge = new Gauge({
  name: "housekeeping_command_consumer_lag",
