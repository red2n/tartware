...
import {
  shutdownCashierCommandCenterConsumer,
  startCashierCommandCenterConsumer,
...
const start = async () => {
  try {
    ...
    if (kafkaEnabled) {
      await startCashierCommandCenterConsumer();
    } else {
...
const shutdown = async (signal: NodeJS.Signals) => {
  ...
  try {
    if (kafkaEnabled) {
      await shutdownCashierCommandCenterConsumer();
