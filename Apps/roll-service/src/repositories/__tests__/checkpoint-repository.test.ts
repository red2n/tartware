...
await runTest("getBackfillCheckpoint returns structured checkpoint when row exists", async () => {
...
await runTest("getBackfillCheckpoint returns null when no rows found", async () => {
  const { stub } = buildQueryStub([]);
...
await runTest("upsertBackfillCheckpoint writes ISO timestamp via provided client", async () => {
