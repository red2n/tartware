...
- **bn.js 4.12.3** — Security fix for CVE-2026-2739 / SNYK-JS-BNJS-15274301 via `@fastify/jwt` -> `fast-jwt` -> `asn1.js`.

## Service Port Map
Canonical dev port assignments (set via `PORT=` env var in root `package.json` dev scripts). After consolidation, 10 standalone services have been merged into host services (see `docs/SERVICE_CONSOLIDATION_TRACKER.md`).

| Port | Service | Dev Script | Hosted Modules | Notes |
|------|---------|------------|----------------|-------|
| 3000 | core-service | `dev:core` | settings-service | |
| 3010 | guests-service | `dev:guests` | guest-experience-service, notification-service | Dual Kafka consumers for notification |
| 3015 | rooms-service | `dev:rooms` | recommendation-service, calculation-service | |
| 3020 | reservations-command-service | `dev:reservations` | | Kafka + outbox |
| 3025 | billing-service | `dev:billing` | cashier-service | |
| 3030 | housekeeping-service | `dev:housekeeping` | | |
| 3035 | command-center-service | `dev:command-center` | roll-service, service-registry | Kafka + outbox |
| 3045 | availability-guard-service | `dev:availability-guard` | | + gRPC on 4400 |
| 3090 | finance-admin-service | `dev:finance-admin` | accounts-service, revenue-service | Kafka consumer |
| 8080 | api-gateway | `dev:gateway` | | Entry point |

- When adding a new service, assign the next port in the sequence (next: **3095**) and add it to `dev:backend`/`dev:stack` in root `package.json`.
...
- Hosted modules run inside their host service process and share the host's port. Their Kafka consumers use separate group IDs.
...
- **Always route requests through the API Gateway (port 8080)**—never call individual services directly (ports 3000–3090, etc.) during testing.
